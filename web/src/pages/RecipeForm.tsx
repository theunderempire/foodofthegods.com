...
export function RecipeForm() {
  ...
  function applyImportedRecipe(recipe: Recipe) {
    setName(recipe.name ?? "");
  ...
  async function handleImportText() {
    ...
    setError("");
    try {
      const recipe = await importRecipeFromText(importText.trim());
      if (!recipe) {
        setError("Failed to parse recipe from text.");
        return;
      }
      applyImportedRecipe(recipe);
    } catch {
    ...
    <div className="page page-narrow">
      ...
      {step === "select" && (
        <div className="import-select">
          ...
          <button className="import-option" onClick={() => setStep("form")}>
            <span className="import-option-icon">&#9998;</span>
            <span className="import-option-label">Enter Manually</span>
            <span className="import-option-desc">Fill in the form yourself</span>
          </button>
        </div>
      )}

      {step === "url" && (
        <div className="import-input-screen">
          {importing ? (
            <div className="page-loading">Importing recipe...</div>
          ) : (
            <>
      ...
      {step === "text" && (
        <div className="import-input-screen">
          {importing ? (
            <div className="page-loading">Parsing recipe...</div>
          ) : (
            <>
              <label htmlFor="importText">Recipe Text</label>
              <textarea
                id="importText"
                className="input"
                value={importText}
